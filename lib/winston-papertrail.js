...
var Papertrail = exports.Papertrail = function (options) {
    ...
    // Open the connection
    connectStreamDebounce();

	// prevent lots and lots of these from being called all at once
	var _connectTimer = null;
	function connectStreamDebounce() {
		if (_connectTimer) {
			clearTimeout(_connectTimer);
		}

		_connectTimer = setTimeout(connectStream, self.connectionDelay);
	}
    ...
    function connectStream() {
        ...
        try {
            function wireStreams() {
                ...
                self.stream.on('end', connectStreamDebounce);
            }
